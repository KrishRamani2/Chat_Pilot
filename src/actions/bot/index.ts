  ...
  try {
    ...
    if (chatBotDomain) {
      ...
      if (customerEmail) {
        ...
        const chatCompletion = await genAI.getGenerativeModel({ model: "gemini-pro" }).generateContent(`
              ...
              ${chat.map(msg => `${msg.role}: ${msg.content}`).join('\n')}
        ...
        if (chatCompletion.response.text().includes('(realtime)')) {
          ...
          if (realtime) {
            const response = {
              role: 'assistant',
              content: chatCompletion.response.text().replace(
                '(realtime)',
                ''
              ),
            }
